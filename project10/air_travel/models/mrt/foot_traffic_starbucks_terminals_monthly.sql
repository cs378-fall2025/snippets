with foot_traffic_starbucks_terminals_monthly as (
  ...
  from
    ...
    from {{ ref('TSA_Traffic') }} t
    join {{ ref('tmp_tsa_checkpoint_terminal') }} ct
      on t.airport_icao = ct.airport_icao and t.security_checkpoint = ct.security_checkpoint
    join {{ ref('Airport') }} a on t.airport_icao = a.icao
    join {{ ref('Airport_Concessions') }} ac on a.icao = ac.airport_icao
    join {{ ref('Airport_Concessions') }} ac2
